...
class UmbrielCapabilityContractTests(unittest.TestCase):
    def test_fixture_provenance_matches_contract_reference(self) -> None:
        ...
        self.assertEqual(metadata["binary"], "build-approved/umbriel")
    ...
    def setUp(self) -> None:
        ...
        self.binary.write_text(textwrap.dedent(f"""\
            ...
            if args == ["msg", "--help"]:
                ...
                raise SystemExit(0)
            if args == ["--version"]:
                print(os.environ.get("NBSHELL_UMBRIEL_VERSION", "umbriel 0.1.0 ({json.loads((FIXTURES / 'fixture.json').read_text())['sourceRevision'][:12]})"))
                raise SystemExit(0)
    ...
    def test_reference_fixture_discovers_every_required_capability(self) -> None:
        ...
        revision = json.loads((FIXTURES / "fixture.json").read_text())["sourceRevision"]
        self.assertEqual(value["runtime"]["version"], f"umbriel 0.1.0 ({revision[:12]})")
