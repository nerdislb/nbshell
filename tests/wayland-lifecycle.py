...
def inside(args):
    ...
    (config / 'config.json').write_text(json.dumps({
        ...
        'rightWidgets': ['notifications'] if args.activity_contract else ['battery'] if args.power_contract else ['control'] if (args.network_contract or args.bluetooth_contract) else ['volume'] if args.audio_contract else ['volume', 'control'] if args.control_contract else (['ai'] if args.startup_ai_widget else []), 'collapsedWidgets': ['clock'],
    }))
    ...
    if args.parity_contract:
        runpy.run_path('/source/tests/parity-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.speedtest_contract:
        runpy.run_path('/source/tests/speedtest-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.calculator_contract:
        runpy.run_path('/source/tests/calculator-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.keys_contract:
        runpy.run_path('/source/tests/keys-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.emoji_contract:
        runpy.run_path('/source/tests/emoji-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.session_contract:
        runpy.run_path('/source/tests/session-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.capture_contract:
        runpy.run_path('/source/tests/capture-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.notification_center_contract:
        runpy.run_path('/source/tests/notification-center-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.osd_contract:
        runpy.run_path('/source/tests/osd-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.displays_contract:
        runpy.run_path('/source/tests/displays-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.plugins_contract:
        runpy.run_path('/source/tests/plugins-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.modules_contract:
        runpy.run_path('/source/tests/modules-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.settings_contract:
        runpy.run_path('/source/tests/settings-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.wallpaper_contract:
        runpy.run_path('/source/tests/wallpaper-picker-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.theme_contract:
        runpy.run_path('/source/tests/theme-lifecycle.py')['instrument'](Path('/work/shell'))
    if args.activity_contract:
        runpy.run_path('/source/tests/activity-lifecycle.py')['instrument'](Path('/work/shell'))
    ...
    if args.network_contract:
        runpy.run_path('/source/tests/network-lifecycle.py')['instrument'](Path('/work/shell'))
...
''' + source[end:]
    ...
    IpcHandler {
        ...
        function headerVisible(): bool {
            return content.mapToItem(root, 0, 0).y >= 0;
        }
    }
''' + source[end:]
...
''' + text[end:])
    ...
    try:
        ...
        time.sleep(2)
        if args.parity_contract:
            runpy.run_path('/source/tests/parity-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.speedtest_contract:
            runpy.run_path('/source/tests/speedtest-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.calculator_contract:
            runpy.run_path('/source/tests/calculator-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.keys_contract:
            runpy.run_path('/source/tests/keys-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.emoji_contract:
            runpy.run_path('/source/tests/emoji-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.session_contract:
            runpy.run_path('/source/tests/session-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.capture_contract:
            runpy.run_path('/source/tests/capture-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.notification_center_contract:
            runpy.run_path('/source/tests/notification-center-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.osd_contract:
            runpy.run_path('/source/tests/osd-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.displays_contract:
            runpy.run_path('/source/tests/displays-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.plugins_contract:
            runpy.run_path('/source/tests/plugins-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.modules_contract:
            runpy.run_path('/source/tests/modules-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.settings_contract:
            runpy.run_path('/source/tests/settings-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.wallpaper_contract:
            runpy.run_path('/source/tests/wallpaper-picker-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.theme_contract:
            runpy.run_path('/source/tests/theme-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.activity_contract:
            runpy.run_path('/source/tests/activity-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.power_contract:
        ...
        if args.bluetooth_contract:
            ...
            return
        if args.network_contract:
            runpy.run_path('/source/tests/network-lifecycle.py')['exercise'](run, launch, wait, ipc, processes, shell, args)
            return
        if args.audio_contract:
...
def main():
    ...
    parser.add_argument('--parity-contract', action='store_true')
    parser.add_argument('--parity-core-only', action='store_true')
    parser.add_argument('--parity-extras-only', action='store_true')
    parser.add_argument('--parity-snapshot', action='store_true')
    parser.add_argument('--speedtest-contract', action='store_true')
    parser.add_argument('--calculator-contract', action='store_true')
    parser.add_argument('--keys-contract', action='store_true')
    parser.add_argument('--emoji-contract', action='store_true')
    parser.add_argument('--session-contract', action='store_true')
    parser.add_argument('--capture-contract', action='store_true')
    parser.add_argument('--notification-center-contract', action='store_true')
    parser.add_argument('--osd-contract', action='store_true')
    parser.add_argument('--displays-contract', action='store_true')
    parser.add_argument('--plugins-contract', action='store_true')
    parser.add_argument('--modules-contract', action='store_true')
    parser.add_argument('--settings-contract', action='store_true')
    parser.add_argument('--wallpaper-contract', action='store_true')
    parser.add_argument('--theme-contract', action='store_true')
    parser.add_argument('--activity-contract', action='store_true')
    ...
    require(not (args.parity_contract or args.speedtest_contract or args.calculator_contract or args.keys_contract or args.emoji_contract or args.session_contract or args.capture_contract or args.notification_center_contract or args.osd_contract or args.displays_contract or args.plugins_contract or args.modules_contract or args.settings_contract or args.wallpaper_contract or args.theme_contract or args.activity_contract or args.notification_contract or args.menu_contract or args.control_contract or args.audio_contract or args.network_contract or args.bluetooth_contract or args.power_contract) or args.pointer_client, 'Native keyboard tests require --pointer-client')
    if args.inside:
        inside(args); return
    ...
    with tempfile.TemporaryDirectory(prefix='nbshell-lifecycle-') as directory:
        ...
        if args.parity_contract or args.speedtest_contract or args.calculator_contract or args.keys_contract or args.emoji_contract or args.session_contract or args.capture_contract or args.notification_center_contract or args.osd_contract or args.displays_contract or args.plugins_contract or args.modules_contract or args.settings_contract or args.wallpaper_contract or args.theme_contract or args.activity_contract or args.notification_contract or args.menu_contract or args.control_contract or args.audio_contract or args.network_contract or args.bluetooth_contract or args.power_contract:
