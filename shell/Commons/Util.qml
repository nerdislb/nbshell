...
// Shared utility helpers, ported from the pinned Omarchy reference so an
// upstream component or plugin can call Util.* unchanged. Pure functions
// only — no state, and no theme coupling. Anything theme-aware belongs on
// Color or Style, anything stateful on a service.
//
// Deliberate deviation: `execDetached` and `execArgv` are NOT ported. Both
// call Quickshell.execDetached, and importing the Quickshell module here would
// break the stock Qt test runner, which cannot load the quickshell-coreplugin
// and therefore fails the whole qs.Commons module. A nbshell surface that
// needs to launch a process uses its own service instead. See
// docs/ui-porting.md.
QtObject {
  id: root
  ...
  // quotes by closing, escaping, and re-opening the literal. Available even
  // though the exec helpers below are not: a caller that builds its own
  // command still needs the quoting.
  ...
  // Layout normalization for bar config consumers. Entries are deep-cloned to
  // decouple from the input config; consumers can mutate without leaking back
