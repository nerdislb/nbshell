...
write_omazen_colors() {
	...
	chmod 600 "$temporary"
	cat >"$temporary" <<EOF
# Generated by nbshell for Omazen. Do not edit manually.
mode = "$mode"
accent = "$accent"
selection = "$selection"
muted = "$muted"
...
run_omazen() {
	local binary
	binary=$(omazen_binary) || die "Omazen is not installed"
...
EOF
	...
	if omazen_configured; then
		write_omazen_colors
		run_omazen sync >/dev/null || printf 'nbshell browser theme: Omazen sync failed\n' >&2
	fi
}
