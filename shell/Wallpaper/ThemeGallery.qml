...
PanelWindow {
    ...
    readonly property var current: ThemeIndex.list[selected] ?? null
    ...
    function select(index) {
        if (!ThemeIndex.list.length)
            return;
        selected = (index + ThemeIndex.list.length) % ThemeIndex.list.length;
    ...
    FocusScope {
        ...
        Keys.onPressed: event => {
            if (event.key === Qt.Key_Home) {
                ...
                root.select(ThemeIndex.list.length - 1);
                event.accepted = true;
            }
        }
        ...
        MotionSurface {
            ...
            Column {
                ...
                ListView {
                    ...
                    width: parent.width
                    height: parent.height - Theme.cellH * 7.2
                ...
                Row {
                    ...
                    Line {
                        ...
                        text: "← → browse  ·  scroll or swipe  ·  Enter apply  ·  Esc close"
                        color: Theme.muted
                        font.pixelSize: Theme.fontCaption
                        elide: Text.ElideRight
                    }
