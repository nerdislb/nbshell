...
- Use colors from the active nbshell theme. Do not hard-code UI colors.
...
- This plugin is bundled and copied by nbshell's installer. Everything tracked
  is therefore part of the nbshell release payload between a user and a working
  mailbox.
...
- Run `make validate` after any QML or behavior change. It runs the node tests,
  source regressions, `qmllint`, and `nbshell plugin validate`.
