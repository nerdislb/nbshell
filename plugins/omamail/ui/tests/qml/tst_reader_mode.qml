  ...
  TestCase {
    ...
    function test_all_modes_keep_the_content_limit_when_zooming() {
      ...
      for (var i = 0; i < modes.length; i++) {
        ...
        verify(body().width <= (modes[i] === "reader" ? reader.readingMeasure : Style.space(800)), modes[i] + " keeps its maximum width")
        reader.width = 420
      ...
      compare(body().width, Math.min(reader.bodyWidth, reader.readingMeasure))
    }
