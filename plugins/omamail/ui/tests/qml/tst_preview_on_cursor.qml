...
Item {
  ...
  TestCase {
    ...
    function window() {
      return having(app, function(it) { return it.title === "Mail" })
    }
    ...
    function test_the_label_picker_stops_a_preview() {
      ...
      // Dismiss through the real keyboard path so this overlay cannot
      // leak into the next test and suppress its cursor movement.
