...
#!/bin/sh
case "$2" in
  https://lists.example.com/*) printf '%s\n' 'lists.example.com:443:203.0.113.20' ;;
  *) printf '%s\n' 'x.example.com:443:203.0.113.21' ;;
esac
STUB
...
contains "the validated destination is pinned against DNS rebinding" "$config" 'resolve = "lists.example.com:443:203.0.113.20"'
...
# A private or unreachable destination must never produce an actionable status.
# The resolver rejects loopback before curl; a public host that later fails to
# answer follows the ordinary no-status path.
unreachable=$(printf '%s %s %s\n' \
  "$(b64 'https://127.0.0.1:9') " "$(b64 'text/plain')" "$(b64 'x=1')" \
  | sh "$script" 2>/dev/null || true)
case "$unreachable" in
  ""|*" 0") ok "a private or unreachable list reports no status at all" ;;
  *) bad "a private or unreachable list reports no status at all"; printf '         actual: %s\n' "$unreachable" ;;
esac
