...
injected_credentials=$(printf 'user:pass\nurl = "file:///etc/passwd"' | base64 | tr -d '\n')
if printf 'imap %s %s %s\n' "$(b64 'imaps://mail.example.org/INBOX')" \
    "$injected_credentials" "$(b64 'UID SEARCH ALL')" \
    | PATH="$work/bin:$PATH" sh "$script" >/dev/null 2>&1; then
  printf '  FAIL a line break entered curl configuration\n'
  failures=$(( failures + 1 ))
else
  printf '  ok   line breaks cannot inject curl configuration\n'
fi
