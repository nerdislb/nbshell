...
grep -Eq 'active:.*windowOpen' BarWidget.qml \
  || fail "the nbshell bar cell must stay active while the mail window is open"
...
if grep -q 'BarIconButton' BarWidget.qml; then
  grep -q 'selectedFillFor' BarWidget.qml \
    || fail "an Omarchy bar icon must use the selected fill, not a glyph colour"
fi
