...
if [ -n "${OMAMAIL_MAILTO_PRINT:-}" ]; then
  printf '%s\n' "nbshell extension open $plugin_id $payload"
  exit 0
fi

command -v nbshell >/dev/null 2>&1 || fail 'omamail: nbshell is not on PATH'
exec nbshell extension open "$plugin_id" "$payload"
