...
command -v python3 >/dev/null 2>&1 || fail 'unsubscribe.sh: python3 is not installed'
...
validate_config_value() {
  ...
  [ "$clean" = "$1" ] || fail 'unsubscribe.sh: line break in curl configuration field'
}
...
validate_config_value "$content_type"
validate_config_value "$body"
...
resolve=$(python3 "$script_dir/resolve-public-url.py" "$url") \
  || fail 'unsubscribe.sh: refusing a host that does not resolve exclusively to public addresses'

escaped_url=$(escape "$url")
escaped_type=$(escape "$content_type")
escaped_body=$(escape "$body")
escaped_resolve=$(escape "$resolve")
