...
command -v python3 >/dev/null 2>&1 || fail 'image-fetch.sh: python3 is not installed'
...
resolve=$(python3 "$script_dir/resolve-public-url.py" "$url") \
  || fail 'image-fetch.sh: refusing a host that does not resolve exclusively to public addresses'
